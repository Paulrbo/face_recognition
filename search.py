...
║                                                                      ║
║  Issu de detection.ipynb — adapté pour le backend FastAPI           ║
║                                                                      ║
...
from projects import csv_path, PROJECT_DIR

DEVICE = "cuda" if torch.cuda.is_available() else "cpu"

# ── Modèles chargés une seule fois au démarrage ───────────────────────────────
# (identiques à ton notebook detection.ipynb)
...
def _load_embeddings(project_id: str) -> tuple[torch.Tensor, pd.DataFrame]:
    """Charge et met en cache les embeddings d'un projet (depuis son CSV)."""
...
def _get_photo_url(project_id: str, filename: str) -> str:
    ...
    DEV  → URL locale via le serveur de fichiers statiques.
    PROD → URL Google Drive (à brancher plus tard).
    """
    if settings.DEV_MODE:
        for f in (PROJECT_DIR / project_id / "photos").rglob(filename):
            rel = f.relative_to(PROJECT_DIR)
            return f"http://localhost:8000/photos/{rel.as_posix()}"
        return ""
    # TODO : URL Google Drive en prod
    return ""
...
def rechercher(
    ...
    Adapté depuis trouver_photos() de detection.ipynb :
    - L'image arrive en PIL (au lieu de numpy BGR via cv2)
    - Le résultat est une liste de dicts (au lieu d'un DataFrame + liste)
    - Pas de matplotlib (c'est le frontend qui affiche)

    Returns:
        None  → aucun visage détecté dans le selfie
        []    → aucune correspondance trouvée (augmente le seuil)
        [...]  → liste de photos matchées
    """
    # Converti PIL → numpy RGB (équivalent de cv2.cvtColor BGR→RGB dans le notebook)
    img_rgb = np.array(img)

    # ── Détection du visage dans le selfie (= photo de référence) ────────────
    # Identique à ton notebook : face = mtcnn(img_reference)
    face = _mtcnn(img_rgb)
    if face is None:
        return None  # aucun visage détecté

    # Si plusieurs visages détectés, prendre le plus grand (index 0)
    if face.dim() == 4:
        face = face[0].unsqueeze(0)
    else:
        face = face.unsqueeze(0)

    # Extraire l'embedding de référence
    with torch.no_grad():
    ...
    # ── Résultats (équivalent de ton resultats + photos_uniques) ──────────────
    ...
    # Une ligne par photo (meilleur match) — identique à ton drop_duplicates
    photos_uniques = (
    ...
    # Ajoute les URLs pour le frontend
